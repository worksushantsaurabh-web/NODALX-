...
function CopyButton({ text, label = 'Copy' }: { text: string; label?: string }) {
  ...
  return (
    <button
      onClick={handleCopy}
      className="inline-flex shrink-0 items-center gap-1.5 rounded-lg bg-black dark:bg-white px-3 py-2 text-xs font-bold text-white transition hover:bg-neutral-800 dark:hover:bg-neutral-200 dark:bg-white dark:text-neutral-950 dark:hover:bg-neutral-200"
    >
...
6. Set it as MAKE_WEBHOOK_URL in your backend:
  ...
  return (
    <div className="space-y-10 animate-fade-in pb-16">
      {/* Header */}
      <div>
        <p className="text-sm font-semibold uppercase tracking-[0.18em] text-black dark:text-white">Integration</p>
        <h1 className="mt-2 text-3xl font-bold tracking-tight text-neutral-950 dark:text-white">Pipeline Setup</h1>
      ...
      <div className="rounded-2xl border border-neutral-200 bg-white dark:border-white/10 dark:bg-white/5">
        <div className="border-b border-neutral-200 p-6 dark:border-white/10">
          <h2 className="flex items-center gap-2.5 text-lg font-bold text-neutral-950 dark:text-white">
        ...
        <div className="space-y-6 p-6">
          ...
          <div className="space-y-3">
            <div className="flex items-center gap-2">
              ...
              <h3 className="text-sm font-bold text-neutral-900 dark:text-white">Submit inquiry</h3>
              <span className="text-xs text-neutral-400">— primary inbound endpoint</span>
            </div>
            <div className="flex items-center gap-2">
              <div className="flex-1 overflow-hidden rounded-xl border border-neutral-200 bg-neutral-50 dark:border-white/10 dark:bg-black/20">
                <code className="block truncate px-4 py-3 text-sm text-neutral-700 dark:text-neutral-300">{inquiryEndpoint}</code>
              </div>
              <CopyButton text={inquiryEndpoint} />
            </div>
            <p className="text-xs text-neutral-400">
              POST JSON with <code className="rounded bg-neutral-100 px-1.5 py-0.5 font-mono text-[11px] text-neutral-600 dark:bg-white/10 dark:text-neutral-300">name</code>, <code className="rounded bg-neutral-100 px-1.5 py-0.5 font-mono text-[11px] text-neutral-600 dark:bg-white/10 dark:text-neutral-300">email</code>, <code className="rounded bg-neutral-100 px-1.5 py-0.5 font-mono text-[11px] text-neutral-600 dark:bg-white/10 dark:text-neutral-300">company</code>, and <code className="rounded bg-neutral-100 px-1.5 py-0.5 font-mono text-[11px] text-neutral-600 dark:bg-white/10 dark:text-neutral-300">message</code> fields.
            </p>
          </div>
          ...
          <div className="space-y-3">
            <div className="flex items-center gap-2">
              ...
              <h3 className="text-sm font-bold text-neutral-900 dark:text-white">Generic webhook</h3>
              <span className="text-xs text-neutral-400">— for custom integrations</span>
            </div>
            <div className="flex items-center gap-2">
              <div className="flex-1 overflow-hidden rounded-xl border border-neutral-200 bg-neutral-50 dark:border-white/10 dark:bg-black/20">
                <code className="block truncate px-4 py-3 text-sm text-neutral-700 dark:text-neutral-300">{webhookEndpoint}</code>
              </div>
      ...
      <div className="rounded-2xl border border-neutral-200 bg-white dark:border-white/10 dark:bg-white/5">
        ...
        <div className="p-6">
          <div className="grid gap-4 sm:grid-cols-4">
            {[
              { step: '1', title: 'Inquiry arrives', desc: 'Your form or CRM posts data to the inquiry endpoint.', icon: Globe, color: 'text-black dark:text-white', bg: 'bg-neutral-100 dark:bg-neutral-800' },
              ...
              <div key={item.step} className="relative flex flex-col items-center text-center">
                {index < 3 && (
                  <div className="absolute left-[calc(50%+28px)] top-6 hidden h-0.5 w-[calc(100%-56px)] bg-neutral-200 sm:block dark:bg-white/10" />
                )}
                <div className={`relative z-10 flex h-12 w-12 items-center justify-center rounded-2xl ${item.bg} ${item.color}`}>
                  <item.icon className="h-5 w-5" />
                </div>
                <h4 className="mt-3 text-sm font-bold text-neutral-900 dark:text-white">{item.title}</h4>
      ...
      <div className="rounded-2xl border border-neutral-200 bg-white p-6 dark:border-white/10 dark:bg-white/5">
        <h2 className="mb-6 text-lg font-bold text-neutral-950 dark:text-white">Supported Integrations</h2>
        <div className="grid gap-4 sm:grid-cols-3">
          {[
            ...
            <a
              ...
              className="group flex items-start gap-4 rounded-2xl border border-neutral-100 bg-neutral-50/50 p-4 transition hover:border-neutral-300 hover:shadow-sm dark:border-white/5 dark:bg-white/5 dark:hover:border-neutral-600"
            >
              <div className={`shrink-0 rounded-xl p-2.5 ${tool.bg} ${tool.color}`}>
                <tool.icon className="h-5 w-5" />
              </div>
              <div className="min-w-0 flex-1">
                <h5 className="flex items-center gap-1.5 text-sm font-bold text-neutral-900 dark:text-white">
                  {tool.name}
                  <ExternalLink className="h-3 w-3 text-neutral-400 opacity-0 transition group-hover:opacity-100" />
                </h5>
                <p className="mt-0.5 text-xs text-neutral-500 dark:text-neutral-400">{tool.desc}</p>
              </div>
            </a>
          ))}
        </div>
