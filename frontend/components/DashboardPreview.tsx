...
export default function DashboardPreview() {
  return (
    <section id="pipeline" className="relative py-24 lg:py-32 bg-white dark:bg-slate-950 overflow-hidden border-t border-slate-100 dark:border-slate-800/50 transition-colors duration-300">
      {/* Subtle Background Glow */}
      <div className="absolute top-0 left-1/2 -translate-x-1/2 w-full max-w-5xl h-full pointer-events-none">
        <div className="absolute top-1/3 right-0 w-[500px] h-[500px] bg-slate-50 dark:bg-teal-900/10 rounded-full blur-3xl animate-pulse-slow"></div>
      </div>
      ...
      <div className="max-w-6xl mx-auto px-6 md:px-12 relative z-10">
        ...
        <div className="relative mx-auto animate-fade-in-up group" style={{ animationDelay: '0.2s' }}>
          ...
          {/* Preview Badge */}
          <div className="absolute -top-4 -right-4 bg-slate-900 dark:bg-white text-white dark:text-slate-900 px-4 py-1.5 rounded-full text-xs font-bold shadow-xl z-20 flex items-center gap-1.5 border border-slate-700 dark:border-slate-200 animate-float">
            <Eye className="w-3.5 h-3.5" />
            Preview Only
          </div>

          {/* Main Dashboard Card - Added pointer-events-none to disable interaction */}
          <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200/80 dark:border-slate-800 shadow-2xl shadow-slate-200/40 dark:shadow-none overflow-hidden flex flex-col transition-all duration-500 group-hover:shadow-3xl group-hover:-translate-y-1 pointer-events-none select-none">
            ...
            <div className="bg-slate-50/80 dark:bg-slate-950/50 border-b border-slate-200/80 dark:border-slate-800 px-5 py-3 flex items-center justify-between backdrop-blur-sm">
              ...
              <div className="flex items-center gap-3">
                <div className="hidden sm:flex items-center gap-2 px-3 py-1.5 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg text-xs text-slate-400 dark:text-slate-500 shadow-sm">
                  <Search className="w-3 h-3" />
                  Search leads...
                </div>
                <div className="w-7 h-7 rounded-full bg-teal-600 dark:bg-teal-500 text-white flex items-center justify-center text-xs font-bold shadow-sm">
                  JD
                </div>
              </div>
            </div>
            ...
            <div className="p-6 md:p-8 bg-slate-50/30 dark:bg-slate-950/30">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-5 mb-8">
                {stats.map((stat, index) => (
                  <div key={index} className="bg-white dark:bg-slate-900 rounded-xl p-5 border border-slate-200/80 dark:border-slate-800 shadow-sm">
              ...
              <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200/80 dark:border-slate-800 shadow-sm overflow-hidden">
                <div className="px-6 py-4 border-b border-slate-200/80 dark:border-slate-800 flex items-center justify-between bg-white dark:bg-slate-900">
                  <h3 className="text-base font-bold text-slate-900 dark:text-white">Recent Inquiries</h3>
                  <button className="flex items-center gap-2 text-xs font-semibold text-slate-600 dark:text-slate-300 px-3 py-1.5 rounded-md border border-slate-200 dark:border-slate-700 shadow-sm">
                ...
                <div className="overflow-x-auto">
                  <table className="w-full text-left border-collapse">
                    <thead>
                      <tr className="bg-slate-50/80 dark:bg-slate-800/50 text-xs uppercase tracking-wider text-slate-500 dark:text-slate-400 font-semibold border-b border-slate-200/80 dark:border-slate-800">
                    ...
                    <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                      {recentInquiries.map((inquiry, index) => (
                        <tr key={inquiry.id} className="bg-white dark:bg-slate-900 animate-fade-in-up" style={{ animationDelay: `${index * 0.1}s` }}>
                          <td className="px-6 py-4 whitespace-nowrap">
                            <div className="flex items-center gap-3">
                              <img src={inquiry.avatar} alt={inquiry.name} className="w-8 h-8 rounded-full border border-slate-200 dark:border-slate-700" />
                              <div>
                                <div className="font-bold text-slate-900 dark:text-white text-sm">{inquiry.name}</div>
                                <div className="text-xs text-slate-500 dark:text-slate-400 font-medium">{inquiry.company}</div>
                              </div>
                            </div>
                          </td>
                          <td className="px-6 py-4 whitespace-nowrap">
                            <div className="flex items-center gap-1.5">
                              ...
                              <span className={`text-sm font-semibold ${inquiry.intent === 'High' ? 'text-orange-600 dark:text-orange-400' : 'text-slate-600 dark:text-slate-400'}`}>
                                {inquiry.intent}
                              </span>
                            </div>
                          </td>
                          ...
                          <td className="px-6 py-4 whitespace-nowrap text-right">
                            <button className="p-1.5 text-slate-400 dark:text-slate-500 rounded-md">
                ...
                <div className="px-6 py-3.5 border-t border-slate-200/80 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-900/50 text-center">
                  <button className="text-xs font-bold text-teal-600 dark:text-teal-400">
