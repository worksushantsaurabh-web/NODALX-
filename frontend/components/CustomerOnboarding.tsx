  ...
  if (isLoading) {
    return (
      <div className="flex flex-col items-center justify-center py-20">
        ...
        <p className="text-slate-500 dark:text-slate-400">Loading your integration status...</p>
      </div>
    );
  }
    ...
    <div className="max-w-5xl mx-auto space-y-8 animate-fade-in">
      ...
      <div className="space-y-3">
        <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-teal-50 dark:bg-teal-500/10 border border-teal-200 dark:border-teal-500/20">
          ...
          <span className="text-xs font-semibold text-teal-700 dark:text-teal-400 tracking-wider">INTEGRATION</span>
        </div>
        <h1 className="text-3xl font-bold text-slate-900 dark:text-white tracking-tight">Your API Key</h1>
        ...
        <div className="p-6 md:p-8 space-y-6">
          {!apiKey ? (
            <div className="flex flex-col items-center justify-center py-12 space-y-6 text-center">
              <div className="h-16 w-16 bg-teal-50 dark:bg-teal-500/10 rounded-full flex items-center justify-center">
                <Key className="h-8 w-8 text-teal-600 dark:text-teal-400" />
              </div>
              <div>
                <h3 className="text-xl font-semibold text-slate-900 dark:text-white mb-2">Generate Your API Key</h3>
            ...
            <div className="space-y-8">
              <div className="flex flex-col md:flex-row md:items-start justify-between gap-6">
                <div className="space-y-4 flex-1">
                  <div className="flex items-center gap-3">
                    {isEditingName ? (
                      ...
                      <div className="flex items-center gap-2">
                        <h3 className="text-xl font-semibold text-slate-900 dark:text-white">{businessName}</h3>
                        <button 
                    ...
                    <span className="px-2.5 py-0.5 bg-slate-100 dark:bg-white/10 text-slate-600 dark:text-slate-300 rounded-md text-xs font-medium capitalize">
                      {plan} Plan
                    </span>
                  </div>
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex items-center gap-2 max-w-lg">
                      <div className="flex-1 flex items-center justify-between bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-lg px-4 py-3 font-mono text-sm text-slate-800 dark:text-slate-200">
                      ...
                      <button 
                        ...
                        className="p-3 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-300 rounded-lg transition-colors flex items-center justify-center focus:outline-none focus:ring-2 focus:ring-teal-500"
                        title="Copy API Key"
                      >
                        {copiedStates['main-key'] ? <Check className="h-5 w-5 text-teal-600 dark:text-teal-400" /> : <Copy className="h-5 w-5" />}
                      </button>
                    </div>
                  </div>
                </div>
                ...
                <div className="flex gap-4 md:flex-col md:w-48 pt-2">
                  <div className="bg-slate-50 dark:bg-slate-800/50 border border-slate-100 dark:border-slate-800 rounded-xl p-4 flex-1">
                    <div className="text-sm text-slate-500 dark:text-slate-400 mb-1 flex items-center gap-2">
                      <Activity className="h-4 w-4" />
                      Total Inquiries
                    </div>
                    <div className="text-2xl font-bold text-slate-900 dark:text-white">{totalInquiries}</div>
                  </div>
                  <div className="bg-slate-50 dark:bg-slate-800/50 border border-slate-100 dark:border-slate-800 rounded-xl p-4 flex-1">
                    <div className="text-sm text-slate-500 dark:text-slate-400 mb-1 flex items-center gap-2">
              ...
              <div className="pt-6 border-t border-slate-100 dark:border-white/10">
                <div className="flex flex-col sm:flex-row items-center gap-4">
                  <button 
                    ...
                    disabled={isTesting}
                    className="w-full sm:w-auto inline-flex items-center justify-center gap-2 px-6 py-3 bg-teal-600 hover:bg-teal-700 text-white rounded-lg font-medium transition-colors disabled:opacity-70 focus:outline-none focus:ring-2 focus:ring-teal-500 focus:ring-offset-2 dark:focus:ring-offset-slate-900"
                  >
      ...
      {apiKey && (
        <div className="bg-white dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-2xl overflow-hidden shadow-sm">
          <div className="p-6 md:p-8 border-b border-slate-100 dark:border-white/10">
          ...
          <div className="flex flex-col md:flex-row">
            <div className="w-full md:w-48 bg-slate-50 dark:bg-slate-900/50 border-r border-slate-200 dark:border-slate-800">
            ...
            <div className="flex-1 relative bg-slate-900">
              <div className="absolute top-4 right-4 z-10">
                <button
                  ...
                  className="flex items-center gap-2 px-3 py-1.5 bg-slate-800 hover:bg-slate-700 text-slate-300 rounded-md transition-colors border border-slate-700 text-sm"
                >
              ...
              <pre className="p-6 overflow-x-auto [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none] text-sm font-mono text-slate-300 h-full w-full min-h-[300px] whitespace-pre-wrap break-all md:break-normal md:whitespace-pre">
      ...
      <div className="bg-white dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-2xl p-6 md:p-8 shadow-sm space-y-4">
        <div>
          <h2 className="text-xl font-bold text-slate-900 dark:text-white">Have an access key?</h2>
      ...
      <div className="bg-white dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-2xl p-6 md:p-8 shadow-sm">
        <h2 className="text-xl font-bold text-slate-900 dark:text-white mb-8">What Happens Next?</h2>
          ...
          {[
            ...
            <React.Fragment key={item.step}>
              <div className="flex flex-col items-center text-center space-y-3 w-full md:w-1/4">
                <div className="h-12 w-12 rounded-full bg-teal-50 dark:bg-teal-900/30 border border-teal-100 dark:border-teal-800 flex items-center justify-center text-teal-600 dark:text-teal-400 relative z-10">
              ...
              {index < arr.length - 1 && (
                <div className="hidden md:flex w-full md:w-auto flex-1 items-center justify-center">
                  <div className="h-[2px] w-full bg-slate-200 dark:bg-slate-700 relative flex items-center justify-center">
                    <ArrowRight className="h-4 w-4 text-slate-300 dark:text-slate-600 absolute bg-white dark:bg-[#121827] px-1" />
                  </div>
                </div>
              )}
              {index < arr.length - 1 && (
